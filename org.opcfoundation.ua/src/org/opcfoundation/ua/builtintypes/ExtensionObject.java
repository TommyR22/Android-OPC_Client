...
public class ExtensionObject {
	...
	public static ExtensionObject encode(
			...
			QualifiedName encodingType, IEncodeableSerializer serializer) throws EncodingException {
		if (encodeable == null)
			return null;
		if (encodingType.equals(QualifiedName.DEFAULT_BINARY_ENCODING))
				return ExtensionObject.binaryEncode(encodeable, serializer);
	...
	public static ExtensionObject encode(
			Structure encodeable,
			QualifiedName encodingType) throws EncodingException {
		return encode(encodeable, encodingType, StackUtils.getDefaultSerializer());
	}
...
//		this.encodeType = EncodeType.Xml;
	...
	throws DecodingException {
		...
		if (object instanceof XmlElement) 
		{
			...
			if (inElement)
				dec.getStartElement();
			T result = (T) serializer.getEncodeable(clazz, dec);
			if (inElement)
				dec.getEndElement();
			return result;
		}
