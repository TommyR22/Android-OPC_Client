...
public class CryptoUtil {
	...
	public static int getAsymmInputBlockSize(SecurityAlgorithm algorithm) throws ServiceResultException {
	...
	public static byte[] asymmEncrypt(byte[] input, Key privKey, SecurityAlgorithm algorithm) throws InvalidKeyException, IllegalBlockSizeException, BadPaddingException, ServiceResultException, NoSuchAlgorithmException, NoSuchPaddingException
	{
		Cipher cipher = Cipher.getInstance(algorithm.getStandardName());
		cipher.init(Cipher.ENCRYPT_MODE, privKey);
		int inputBlock = CryptoUtil.getAsymmInputBlockSize(algorithm);
	 ...
	 * 
	 * @param algorithm UA Specified algorithm 
	 * @return Cipher
...
//				return Cipher.getInstance("RSA/NONE/OAEPWithSHA1AndMGF1Padding");
        ...
        throw new ServiceResultException(StatusCodes.Bad_SecurityPolicyRejected, "Unsupported asymmetric signature algorithm: "+algorithm);
    }
	...
	throws ServiceResultException
	{
        SecretKeySpec keySpec = new SecretKeySpec(secret, algorithm.getStandardName() );
        Mac hmac;
        try {
			hmac = Mac.getInstance(algorithm.getStandardName(), getSecurityProviderName());
			hmac.init(keySpec);
	 ...
	 * @throws ServiceResultException Bad_SecurityPolicyRejected algorithm not supported
	 */
	public static int getSignatureSize(SecurityAlgorithm signatureAlgorithm, Key key)
	throws ServiceResultException
	{
		if (signatureAlgorithm==null) return 0;
		if (signatureAlgorithm.getType().equals(SecurityAlgorithm.AlgorithmType.SymmetricSignature))
...
//			return 256/8;
		...
		throw new ServiceResultException(StatusCodes.Bad_SecurityPolicyRejected, signatureAlgorithm.getUri());
	}
...
//		}
		...
		throw new ServiceResultException(StatusCodes.Bad_SecurityPolicyRejected, algorithm.getUri());
	}	
	
	/**
	 * Get plain text block (=input) size in bytes
	 * 
	 * @param securityAlgorithm algorithm
	 * @param key Optional, required for asymmetric encryption algorithms  
	 * @return cipher block size
	 * @throws ServiceResultException Bad_SecurityPolicyRejected algorithm not supported
	 */
	...
	{
		// No security
		if (securityAlgorithm==null) return 1;
	 ...
	 * 
	 * @param algorithm asymmetric encryption algorithm or null (=no encryption)
...
//		LOGGER.debug("createNonce: nonce=" + nonce);
	...
	static {
		...
		try {
			...
			// If not done, it will self-seed, which in some Linux systems can take a long while
	...
	public static byte[] createNonce(SecurityAlgorithm algorithm) throws ServiceResultException {
	...
	/**
	 * Convenience method for "displaying" a hex-string of a given byte array. Calls {@link #toHex(byte[], int)} with bytesPerRow=0 (no line breaks)
	 * 
	 * @param bytes
	 *            the byte array to "display"
	 */
	public static String toHex(byte[] bytes) {
		return toHex(bytes, bytes.length > 64 ? 64 : 0);
	}
	/**
	 * Convenience method for "displaying" a hex-string of a given byte array. Breaks the string to lines, if bytesPerRow > 0.
	 * 
	 * @param bytes the byte array to "display"
	 * @param bytesPerRow number of bytes to include on a text row. If it is 0, no line breaks are added.
	 * 
	 */
	...
	/**
	 * The Preferred Security Provider name. Will check if a Spongy Castle (on Android) or
	 * Bouncy Castle provider is already available or if such can be initialized
	 * from the respective class.
	 * <p>
	 * If none of these is available will default to SunJCE or the first initialized provider, if SunJCE is not available either.
	...
	private static Provider hasClass(String className) {
		try {
			Class<?> providerClass = CryptoUtil.class.getClassLoader().loadClass(className);
			try {
				Provider provider = (Provider) providerClass
						.getConstructor().newInstance();
				Security.addProvider(provider);
				LOGGER.info("SecurityProvider initialized from " + providerClass.getName());
				return provider;
			} catch (Exception e) {
				throw new RuntimeException(
						"Cannot add Security provider class=" + providerClass.getName(), e);
			}
	 ...
	 * 
	 * @param cipherSuiteSet1 enabled cipher suites
	 * @param cipherSuiteSet2 filter list
	 * @param omitProtocol if true the first 3 charaters are ignored in compare
	 * @return an intersection of suites
	 */
	public static String[] getCipherSuiteIntersection(String[] cipherSuiteSet1, String[] cipherSuiteSet2, boolean omitProtocol) 
	{
		List<String> result = new ArrayList<String>( Math.max(cipherSuiteSet1.length, cipherSuiteSet2.length) );
		Set<String> set = new TreeSet<String>();
		for ( String cs : cipherSuiteSet2 ) set.add( omitProtocol ? cs.substring(3) : cs );
		for ( String cs : cipherSuiteSet1 ) {
			if ( set.contains( omitProtocol ? cs.substring(3) : cs ) ) result.add(cs);
		}
		return result.toArray( new String[result.size()] );
	}

	public static String[] filterCipherSuiteList(String[] cipherSuiteSet, String[] cipherSuitePatterns)
	{
		...
		for (int i=0; i<c; i++) patterns[i] = Pattern.compile(cipherSuitePatterns[i]);
	...
	public static byte[] base64Decode(String string) {
		if (getSecurityProviderName().equals("SC"))
			return SpongyCastleUtils.base64Decode(string);
		else if (getSecurityProviderName().equals("BC"))
			return BouncyCastleUtils.base64Decode(string);
		else 
		  return SunJceUtils.base64Decode(string);
	}
	

	public static String base64Encode(byte[] bytes) {
		if (getSecurityProviderName().equals("SC"))
			return SpongyCastleUtils.base64Encode(bytes);
		else if (getSecurityProviderName().equals("BC"))
			return BouncyCastleUtils.base64Encode(bytes);
		else 
			return SunJceUtils.base64Encode(bytes);
