...
public class ListenableSocketChannel {
	static Logger logger = Logger.getLogger(ListenableSocketChannel.class);
	...
	{
		if (logger.isDebugEnabled())
			logger.debug("close: channel.isRegistered()="+channel.isRegistered());
		if (channel.isRegistered())
			selector.unregister(channel);
		if (logger.isDebugEnabled())
			logger.debug("close: channel.isOpen()="+channel.isOpen());
		if (channel.isOpen())
		try {
			try {
				channel.close();
