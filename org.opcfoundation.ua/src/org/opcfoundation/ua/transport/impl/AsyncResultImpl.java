...
public class AsyncResultImpl implements AsyncResult {
	...
	/** Logger */
	static Logger logger = Logger.getLogger(AsyncResultImpl.class);
	static Logger listenerLogger = Logger.getLogger(ResultListener.class);
	...
	public void setError(final ServiceResultException error)
	{
		synchronized(this) {
			...
			this.error = error;
			final ResultListener l = listener;
			if (l != null) {
...
//		logger.debug("finish");
	...
	public void setResultSync(Object result) 		
	throws RuntimeException
	{
		synchronized(this) {
			if (isFinished()) return;
	...
	@Override
	public Object waitForResult(long timeout, TimeUnit unit)
			throws ServiceResultException {
		try {
...
//		logger.debug("result:"+ result);
	 ...
	 * @param source
	 */
	public void setSource(AsyncResult source)
	{
		source.setListener(new ResultListener() {
			@Override
