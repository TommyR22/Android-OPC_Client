...
public class OpcTcpServerConnection extends AbstractServerConnection {
	...
	/** Logger */
	private final static Logger logger = Logger.getLogger(OpcTcpServerConnection.class);		
	...
	OpcTcpServerConnection(OpcTcpServer endpointServer, AsyncSocketImpl s) {
		...
		this.encoderCtx = EncoderContext.getDefault();
...
//			}				
	...
	protected void handleOpenSecureChannelRequest(InputMessage mb) throws ServiceResultException {
		...
		if (msg == null)
			synchronized (this) {
				...
				log.warn("InputMessage has error", e);
	...
	public void addConnectionListener(IConnectionListener listener) {
		logger.debug("addConnectionListener");
	...
	public void removeConnectionListener(IConnectionListener listener) {
		logger.debug("removeConnectionListener");
	...
	MonitorListener inputListener =
		new MonitorListener() {
			...
			public void onStateTransition(IStatefulObject<BufferMonitorState, ?> sender,
				...
				{
					log.error("Unexpected trigger state "+newState);
					return;
				}
				...
				if (is.available()>=chunkSize) {
					...
					try {
						try {
							...
							logger.warn("Error in handlChunk", e);
						...
						try {
							...
							log.warn("Could not send ErrorMessage:",e1);
						}
						setError(se);
					}					
