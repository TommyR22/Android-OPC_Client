...
public class OpcTcpServerSecureChannel extends AbstractServerSecureChannel implements ServerSecureChannel {

	/** Logger */
	static Logger logger = Logger.getLogger(OpcTcpServerSecureChannel.class);
	...
	public void close() {				
		...
		logger.info("Channel closed: Id="+ secureChannelId + ", open channels="+ connection.secureChannels.size());
		return;
	}
	...
	private SecurityToken createToken(OpenSecureChannelRequest req, InputMessage mb) throws ServiceResultException
	{
		...
		SecurityAlgorithm algo = securityConfiguration.getSecurityPolicy().getAsymmetricEncryptionAlgorithm();
	...
	private void sendOpenChannelResponse(InputMessage mb,
		...
		connection.secureChannels.put(secureChannelId, this);
...
//		}
		...
		if (messageMode.hasSigning() || EndpointUtil.containsSecureUserTokenPolicy(getServer().getUserTokenPolicies()))
			...
			logger.warn("Client defines a certificate although SecurityPolicy.NONE is defined");
