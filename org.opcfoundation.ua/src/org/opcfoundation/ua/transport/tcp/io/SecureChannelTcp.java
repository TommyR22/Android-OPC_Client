...
public class SecureChannelTcp implements IMessageListener, IConnectionListener, ITransportChannel, org.opcfoundation.ua.transport.SecureChannel {
	...
	throws ServiceFaultException, ServiceResultException {
		...
		try {
			try {
				...
				if (logger.isDebugEnabled())
					logger.debug("serviceRequest: Message sent, requestId=" + req.requestId
	 ...
	 * @return result asynchronous result object
	 */	
	public AsyncResult serviceRequestAsync(ServiceRequest request, long operationTimeout)
	{
...
//		logger.debug("scheduling async request to another thread: "+req.requestId);
	...
	throws ServiceResultException
	{
		...
		if (logger.isDebugEnabled())
			logger.debug("createSecureChannel: renew="+renew + " channel="+channel);
		if (channel == null)
		...
		SecurityAlgorithm algorithm = policy.getAsymmetricEncryptionAlgorithm();
	...
	throws ServiceResultException {
		...
		for (;;) {
			...
			if (currentTime > req.timeoutTime) {
				if (logger.isDebugEnabled())
					logger.debug("Request id=" + req.requestId + " timeouted "
							+ elapsedTime + "ms elapsed. timeout at "
							+ (req.timeoutTime - req.startTime) + " ms");
				requests.remove(req.requestId);
				req.result.setError(BAD_TIMEOUT);
				continue;
			}
			...
			if (message instanceof OpenSecureChannelResponse == false) {
				 {
					...
					if (serviceFault != null
						...
						logger.info("ServiceFault=" + serviceFault.toString());
					else {
						logger.warn(secureChannelId
								+ " Unidentified message, RequestId="
								+ requestId + ", type="
								+ message.getClass().getSimpleName() + "!");
						if (serviceFault != null)
							logger.warn("ServiceFault=" + serviceFault.toString());
					}
				}
			}
			return; // not for us
