...
class HttpsClientPendingRequest implements Runnable {
	...
	private static Logger logger = Logger.getLogger(HttpsClientPendingRequest.class);
	...
	public void run() {
		try {
			...
			try {
				...
				result.setError( new ServiceResultException(StatusCodes.Bad_SecurityPolicyRejected, e, "Could not negotiate a TLS security cipher or server did not provide a certificate. E.g. the required TLS 1.2 cipher is not supported by JSSE."));
				return;
			}
