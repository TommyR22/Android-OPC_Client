...
class HttpsServerPendingRequest extends EndpointServiceRequest<ServiceRequest, ServiceResponse> implements Runnable {
	/** Logger */
	private final static Logger logger = Logger.getLogger(HttpsServerPendingRequest.class);
	...
	public void run() {
		...
		try {			
			...
			dec.setEncoderContext( endpoint.encoderCtx );
    ...
    void sendResponse(int statusCode, IEncodeable responseObject)
    {
    	try {
	    	...
	    	if ( responseObject != null ) {
	    		try {
	    	    	if (logger.isTraceEnabled()) {
	    	    		logger.trace("sendResponse: requestId=" + requestId + " statusCode=" + statusCode + " responseObject=" + responseObject);
	    	    	} else
	    	    	if (logger.isDebugEnabled()) {
	    	    		logger.debug("sendResponse: requestId=" + requestId + " statusCode=" + statusCode + " responseObject=" + responseObject.getClass().getSimpleName());
	    	    	}
	    	    	if (logger.isDebugEnabled() && channel.lastConnection != null) {
						NHttpServerConnection nHttpServerConnection = channel.lastConnection.conn;
						logger.debug("sendResponse: timeout="
								+ httpExchange.getTimeout() + " " + nHttpServerConnection.getSocketTimeout() + " context="
								+ nHttpServerConnection.getContext());
					}
	        		EncoderCalc calc = new EncoderCalc();
	    			calc.setEncoderContext( endpoint.encoderCtx );
					calc.putMessage( responseObject );
		    		int len = calc.getLength();
		    		byte[] data = new byte[ len ];
		    		BinaryEncoder enc = new BinaryEncoder( data );
		    		enc.setEncoderContext( endpoint.encoderCtx );
	    	...
	    	if (logger.isDebugEnabled()) {
	    		logger.debug("sendResponse: " + responseHandle + " length=" + responseHandle.getEntity().getContentLength());
	    	}
