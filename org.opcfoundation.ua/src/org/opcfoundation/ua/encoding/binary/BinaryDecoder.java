...
//		return new XmlElement(str);
	...
	throws DecodingException 
	{
		try {
			...
			if (encodingByte==1) {						
				Class<? extends IEncodeable> clazz = ctx.getEncodeableSerializer().getClass(typeId);
    ...
    throws DecodingException	
	{
		try {
			...
			if (encodingByte==1) return new ExtensionObject(typeId, getByteString(null));
			if (encodingByte==2) return new ExtensionObject(typeId, getXmlElement(null));
			throw new DecodingException("Unexpected encoding byte ("+encodingByte+") in ExtensionObject");
		} catch (IOException e) {
			throw toDecodingException(e);
		}
    ...
    throws DecodingException	
	{
		try {
			...
			if (!multiDimension) return new Variant(value);
		
			// Build multi-dimension
			try {
				return new Variant( MultiDimensionArrayUtils.demuxArray((Object[]) value, dims) );
    ...
    throws DecodingException	
	{
		...
		Class<T> clazz = (Class<T>) ctx.getEncodeableSerializer().getClass(id);
