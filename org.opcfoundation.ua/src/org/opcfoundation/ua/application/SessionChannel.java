...
public class SessionChannel extends ChannelService implements RequestChannel {
	...
	public ActivateSessionResponse activate(UserIdentityToken identity, SignatureData identitySignature) throws ServiceResultException
	{
		...
		if (!MessageSecurityMode.None.equals(channel.getMessageSecurityMode())) {
			...
			try {
				clientSignature = CertificateUtils.sign(signerKey,
						algorithm, dataToSign);
		...
		asreq.setUserIdentityToken( ExtensionObject.binaryEncode( identity ) );
	...
	public AsyncResult closeAsync() {
		...
		AsyncResult r = CloseSessionAsync(null, true);
		r.setListener(new ResultListener() {
			@Override
			public void onCompleted(Object x) {
				AsyncResult res = channel.closeAsync();
...
//		close();		
	...
	public IEncodeable serviceRequest(ServiceRequest serviceRequest) throws ServiceResultException
	{
		...
		if (LOGGER.isTraceEnabled())
			LOGGER.trace("serviceRequest: Request="+serviceRequest.getClass().getSimpleName() + " SecureChannelId="+channel.getSecureChannelId());
