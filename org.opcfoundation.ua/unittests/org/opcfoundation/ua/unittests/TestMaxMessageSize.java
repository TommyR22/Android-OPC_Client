	...
	void executeTests(String uri, SecurityMode mode, boolean testClient, boolean testServer, HttpsSecurityPolicy httpsSecurityPolicy) throws Exception
	{
		...
		for (int i=2; i<expectedSuccess.length; i++) {		
			...
			try {
				...
				KeyPair myServerKeypair = ExampleKeys.getKeyPair("server", this.keySize);
				KeyPair myServerHttpsKeypair = ExampleKeys.getKeyPair("https_server", this.keySize);
				...
				// Create client
				KeyPair myClientKeys = ExampleKeys.getKeyPair("client", this.keySize);
				KeyPair myClientHttpsKeys = ExampleKeys.getKeyPair("https_client", this.keySize);
